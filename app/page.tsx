...
export default async function Home() {
  ...
  return <main id="top" className="page-managed" style={pageThemeStyle(page)}>
    ...
    <section className="section home-products"><div className="container"><div className="section-head"><div><div className="section-index">Сравнение систем</div><h2>{pageText(page,"productsHeading","Четыре типа перекрытий")}</h2></div><p>{pageText(page,"productsText","Цена указана за базовый комплект материалов. Монтаж, доставка и окончательная комплектация рассчитываются по проекту.")}</p></div><div className="comparison-scroll-hint" aria-hidden="true"><span>←</span> Проведите по таблице <span>→</span></div><div className="comparison-wrap"><table className="product-comparison"><thead><tr><th>Система</th><th>Толщина</th><th>Пролёт</th><th>Нагрузка</th><th>Вес</th><th>Цена от</th></tr></thead><tbody>{products.map((product)=><tr key={product.name}><th><span>{product.name.replace("МАРКО-ГАЗОБЕТОН ","")}</span><small>{product.scenario}</small></th><td>{product.thickness}</td><td>{product.span}</td><td>{product.capacity}</td><td>{product.weight}</td><td><b>{product.price}</b></td></tr>)}</tbody></table></div><div className="comparison-note"><span>В базовый комплект входят несущие балки, газобетонные блоки и спецификация материалов.</span><Link className="text-link" href="/prices">Подробные характеристики <UiIcon name="arrow"/></Link></div></div></section>

    <section className="section object-map"><div className="container map-heading"><div><div className="section-index light">География</div><h2>{pageText(page,"objectsHeading","Реализованные объекты")}</h2></div><div className="map-heading-copy"><p>{pageText(page,"objectsText","Выберите регион, приблизьте карту и откройте метки выполненных проектов.")}</p><Link className="text-link light-link" href="/objects">Перейти к объектам <UiIcon name="arrow"/></Link></div></div><div className="map-frame map-frame-full"><ObjectsMap compact projects={projects}/></div></section>

    <section className="section project-brief"><div className="container contact-wrap"><div className="contact-copy"><div className="section-index light">Расчёт проекта</div><h2>{pageText(page,"formHeading","Отправьте план перекрытия")}</h2><p>{pageText(page,"formText","Инженеру достаточно PDF, DWG, фотографии плана или эскиза с размерами. Предварительную оценку дадим в течение часа, подробный расчёт — до одного рабочего дня.")}</p><div className="brief-benefits"><div><UiIcon name="check"/><span><b>Предварительная оценка за 1 час</b><small>ориентир по материалам и стоимости</small></span></div><div><UiIcon name="check"/><span><b>Подбор системы</b><small>толщина, пролёты и нагрузка</small></span></div><div><UiIcon name="check"/><span><b>Консультация по монтажу</b><small>монтаж, шеф-монтаж или самостоятельная сборка</small></span></div></div></div><div className="contact-form"><h3>Данные для расчёта</h3><ProjectForm/></div></div></section>

    <section className="section home-news"><div className="container"><div className="section-head"><div><div className="section-index">Новости</div><h2>{pageText(page,"newsHeading","Проекты и жизнь компании")}</h2></div><Link className="text-link" href="/news">Все новости <UiIcon name="arrow"/></Link></div><div className="news-grid">{newsItems.slice(0,2).map((item)=><Link key={item.title} href={`/news/${item.slug}`}><small>{item.date}</small><h3>{item.title}</h3><p>{item.excerpt}</p><b>Подробнее <UiIcon name="arrow" size={17}/></b></Link>)}</div></div></section>

    <section className="section compact-cta"><div className="container"><div><div className="eyebrow"><span/>Ваш проект</div><h2>{pageText(page,"ctaHeading","Отправьте план — инженер подготовит расчёт за 1 день")}</h2></div><LeadButton>{pageText(page,"ctaButton","Отправить план перекрытия")} <UiIcon name="arrow"/></LeadButton></div></section>
  </main>;
}
